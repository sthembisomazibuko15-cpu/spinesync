...
const SCRIPTS = [
  `${CDN}/@mediapipe/pose/pose.js`,
  `${CDN}/@mediapipe/camera_utils/camera_utils.js`,
  `${CDN}/@mediapipe/drawing_utils/drawing_utils.js`,
];

// MediaPipe's JS solutions (Pose, Camera, drawing utils) are published for
// direct <script> tag use and don't bundle reliably through Vite/Rollup, so
// we load them from their CDN at runtime and use the resulting globals
// (window.Pose, window.Camera, window.drawConnectors, ...) instead of
// importing the npm packages.
let loadPromise = null;
function loadMediaPipeScripts() {
  if (loadPromise) return loadPromise;
  loadPromise = Promise.all(SCRIPTS.map(src => new Promise((resolve, reject) => {
...
export default function MovementScreen({ onResult }) {
  ...
  useEffect(() => {
    ...
    loadMediaPipeScripts().then(() => {
      if (cancelled) return;
      const pose = new window.Pose({ locateFile: (f) => `${CDN}/@mediapipe/pose/${f}` });
      pose.setOptions({ modelComplexity: 1, smoothLandmarks: true, minDetectionConfidence: 0.5, minTrackingConfidence: 0.5 });
      pose.onResults(onPoseResults);
      poseRef.current = pose;
      setReady(true);
    }).catch(err => setError(`Could not load movement analysis engine: ${err.message}`));
    return () => { cancelled = true; poseRef.current?.close?.(); cameraRef.current?.stop?.(); cancelAnimationFrame(rafRef.current); };
  ...
  function onPoseResults(results) {
    ...
    if (results.poseLandmarks) {
      window.drawConnectors(ctx, results.poseLandmarks, window.POSE_CONNECTIONS, { color: C.bio, lineWidth: 3 });
      window.drawLandmarks(ctx, results.poseLandmarks, { color: C.ore, lineWidth: 1, radius: 3 });
  ...
  if (!ready && !error) return <div style={{ fontFamily:font, fontSize:"0.8rem", color:C.seam }}>Loading movement analysis engine…</div>;

  return (
    <div style={{ fontFamily: font }}>
      {!mode && ready && <div style={{ display:"flex", gap:"0.8rem", flexWrap:"wrap" }}>
