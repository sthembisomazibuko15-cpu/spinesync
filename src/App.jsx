...
function ProfileNotes({worker,onUpdate}) {
  ...
  const notes=worker.clinicalNotes||[];
  const add=()=>{if(!note.trim())return;onUpdate({...worker,clinicalNotes:[{text:note,date:new Date().toLocaleDateString("en-ZA"),author:"Attending BK"},...notes]});setNote("");};
  return <div>
    <Ey>Clinical Notes Log</Ey>
    <textarea value={note} onChange={e=>setNote(e.target.value)} placeholder="Add a clinical note, session observation, or follow-up action..." rows={4} style={{width:"100%",background:C.dust,border:"1px solid "+C.slate,color:C.day,padding:"0.65rem",fontSize:"0.86rem",fontFamily:font,resize:"vertical",outline:"none",borderRadius:"2px",marginBottom:"0.5rem"}}/>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:"0.6rem"}}>
      {notes.length===0?<div style={{color:C.seam,textAlign:"center",padding:"2rem"}}>No clinical notes yet.</div>
